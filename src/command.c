...
gchar *get_help() {
    ...
    // Usage
    g_string_append_printf (string, "Usage: %s <portal> <command> [OPTION]...\n\n", prog_name);

    g_string_append(string, "TODO: Main help text...\n\n");

    g_string_append_printf(string, "To view options for a given portal or command, use: %s <portal> --help\n\n", prog_name);
    ...
    for (int i = 0; i < cli_len; i++) {
        ...
        if (len > longest_portal) longest_portal = len;
    }
...
gchar *get_help_from_portal(Portal *portal) {
    ...
    g_string_append(string, "To view options for a given command, run the command with the --help option.\n\n");
...
gchar *get_help_from_command(Command *command)
{
    ...
    g_string_append_printf(string, "Usage: %s %s %s [OPTION]... %s\n", prog_name, owner->name, command->name, command->usage);
    ...
    for (int i = 0; i < CLI_MAX_OPTIONS; i++) {
        ...
        g_free(left_text);
        if (len > longest_option) longest_option = len;
    }
    ...
    for (int i = 0; i < CLI_MAX_OPTIONS; i++) {
        ...
        gint right_index = 120;// FIXME use get_columns(), but cache it outside the loop
